...
if ($_POST["opt"] === "ANULA") {
    $query = $mysqli->query("call spAnulaRecibidos('" . $_POST["correlativo"] . "','" . $_POST["motivo"] . "')");
}
...
if($_POST["opt"]==="PAGA"){ 
    $query=$mysqli->query( "call spPagaGiro('".$_POST["idgiro"]."','".$_POST["nrooperacion"]."','".$_POST["usuaentrega"]."','".$_POST["sucuentrega"]."',
                            '".$_POST["datospago"]."','".$_POST["monto"]."','".$_POST["idempresa"]."','".$_POST["opsql"]."')");
}
...
if($_POST["opt"]==="B") 
/* Recupera datos de sucursal cuando se usa con ajax...done()*/
{ 
...
if($_POST["opt"]==="BUSCASUCU"){ 
   /*  $query=$mysqli->query(" SELECT cod_sucursal,nom_sucursal,dir_sucursal from sucursal WHERE nom_sucursal LIKE CONCAT('%','".$_POST["nomsucu"]."','%') and idempresa='".$_POST["idempresa"]."' order by nom_sucursal "); */    
    $query=$mysqli->query(" CALL spBuscaSucursal('".$_POST["nomsucu"]."','".$_POST["idempresa"]."');");
    $datos=array();
    while ($recibidos=$query->fetch_array()){
    $datos[]=array(        
...
if ($_POST["opt"] === "FLUJOSUCURSALES") {
    $query = $mysqli->query(" call spFlujoEfectivoCaja_('".$_POST["mitoken"]."') ");
    $datos = array();
...
if ($_POST["opt"] === "EGRESOSXSUCURSAL") {
    $query = $mysqli->query(" call spRptGastosxSucursal_('".$_POST["fechai"]."','".$_POST["fechaf"]."','".$_POST["mitoken"]."','".$_POST["idempresa"]."') ");
    $datos = array();
    while ($col = $query->fetch_array()) {
        $datos[] = array(            
            "respuesta"=>$col["respuesta"]            
        );
    }
    $json = json_encode($datos);
    echo ( $json );
...
{    
    $result=$mysqli->query(" call spRptIngresosxSucursal_('".$_POST["fechai"]."','".$_POST["fechaf"]."','".$_POST["mitoken"]."','".$_POST["idempresa"]."') ");
    $mirpta = new stdClass();
    if($result->num_rows > 0){
        $fila = $result->fetch_array();
        $mirpta->rpta = $fila["respuesta"];       
    }
    echo json_encode($mirpta);
}

if ($_POST["opt"] === "COD_ACREDITA") {
    $codigo_acredita = intval($_POST["codacredita"]);
    $mnts = 1;
    $query = $mysqli->query("call spAcreditador('" . $_POST["nusuario"] . "','" . $_POST["codsucu"] . "',$codigo_acredita,$mnts,'" . $_POST["opcion"] . "','" . $_POST["opcion"] . "')");
    $datos_ca = array();
    while ($valor_ca = $query->fetch_array()) {
        $datos_ca[] = array
        (
            "codigo"    => $valor_ca["codigo"],
            "mensaje"   => $valor_ca["mensaje"]
        );
    }
    $json = json_encode($datos_ca);
    echo ($json);
}

if ($_POST["opt"] === "COD_ACTUALIZA") {
    $codigo_acredita = intval($_POST["codacredita"]);
    $mnts = 1;
    $query = $mysqli->query("call spAcreditador('" . $_POST["nusuario"] . "','" . $_POST["codsucu"] . "',$codigo_acredita,$mnts,'" . $_POST["opcion"] . "','" . $_POST["opcion"] . "')");
    $datos_ca = array();
...
if ($_POST["opt"] === "ELIMINA_VOUCHER") {
    $query = $mysqli->query("call spEliminaVoucher('" . $_POST["nombrearchivo"] . "')");
    
}

if ($_POST["opt"] === "cuenta_cliente") {
    $query = $mysqli->query("call spActualizaCuenta_Clientes('" . $_POST["idcuenta"] . "','" . $_POST["idcliente"] . "','" . $_POST["nrocuenta"] . "','" . $_POST["opt"] . "')");
    $datos = array();
    while ($valor = $query->fetch_array()) {
        $datos[] = array
        (
            "codigo"        => $valor["idcuenta"],
            "mensaje"       => $valor["nrocuenta"],
            "desc_banco"    => $valor["desc_banco"]
        );
    }
    $json = json_encode($datos);
    echo ($json);
...
if ($_POST["opt"] === "ListaCuentaCliente") {

    $nrocuenta=$_POST["nrocuenta"];
    $idbanco=intval($_POST["idbanco"]);
    $idcliente=intval($_POST["idcliente"]);
    $query = $mysqli->query("call spActualizaCuenta_Clientes('" . $_POST["idcuenta"] . "','" . $_POST["idcliente"] . "','" . $_POST["nrocuenta"] . "','" . $_POST["opcion"] . "')");
    $datos = array();
    while ($valor = $query->fetch_array()) {
      $datos[] = array(
        "id_cuenta"      => $valor["id_cuenta"],
        "nrocuenta"     => $valor["nrocuenta"],
        "iniciales"     => $valor["iniciales"],
        "desc_banco"    => $valor["desc_banco"]
      );
    }
    $json = json_encode($datos);
