...
/* if($_GET["opcion"]==="AUTOCOMPLETA") */ {
  ...
  if ($_POST["opcion"] === "U") {
      if (!empty($_POST["idbanco"])){
          $idbanco = (int) $_POST["idbanco"];
      } else { $idbanco = 0; }
    $idusuario = (int) $_POST["idusuario"];

    $query = $mysqli->query("call spInsertaListaCuentas('" . $_POST["nrocuenta"] . "',$idbanco,'0',$idusuario,$idusuario,'" . $_POST["opcion"] . "',
                            '" . $_POST["opproce"] . "','" . $_POST["codempresa"] . "')");
    $datos = array();
    while ($cierre = $query->fetch_array()) {
      $datos[] = array(
  ...
  if ($_POST["opcion"] === "C"){
    ...
    $query = $mysqli->query("call spInsertaListaCuentas('" . $_POST["nrocuenta"] . "',$idbanco,$idcliente,0,'" . $_POST["usuamodifica"] . "','" . $_POST["opcion"] . "','" . $_POST["opproce"] . "','" . $_POST["idempresa"] . "')");
  ...
  if ($_POST["opcion"] === "AGENTES") {
    $query = $mysqli->query(" select b.idbanco,c.nrocuenta,b.iniciales,b.desc_banco from bancos b left JOIN cuentas c on b.idbanco=c.idbanco where b.grupo='A' and b.anulado='N' and c.anulado='N' and c.idempresa='" . $_POST["idempresa"] . "'; ");
    $datos = array();
    while ($cierre = $query->fetch_array()) {
      $datos[] = array(
        "idbanco" => $cierre["idbanco"],
  ...
  if ($_POST["opcion"] === "AGENTES_DIARIO") {
    $query = $mysqli->query(" select c.nrocuenta,b.iniciales,b.desc_banco from bancos b right JOIN cuentas_empresa c on b.idbanco=c.idbanco where b.grupo='A' and b.anulado='N' and c.anulado='N' and c.idempresa='" . $_POST["idempresa"] . "' order by 2; ");
    $datos = array();
    while ($cierre = $query->fetch_array()) {
      $datos[] = array(
        "nrocuenta" => $cierre["nrocuenta"],
  ...
  if ($_POST["opcion"] === "INSERTA") {
    $query = $mysqli->query("call spInsertaListaBancos ('".$_POST["descripcion"]."','".$_POST["iniciales"]."','".$_POST["direccion"]."','".$_POST["web"]."','".$_POST["telefono"]."',
                            '".$_POST["grupo"]."','".$_POST["nrocuenta"]."','".$_POST["idempresa"]."','".$_POST["usuario"]."','".$_POST["usuario_modifica"]."')");
    $datos = array();
    while ($lbanco = $query->fetch_array()) {
      $datos[] = array(
        "idbanco" => $lbanco["idbanco"],
