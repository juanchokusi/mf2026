...
if ($_POST["opcion"] === "MOSTRAR_G") {
    ...
    while ($col = $query->fetch_array()) {
        $datos[] = array(
            ...
            "traslado_efec_agte" => $col["traslado_efec_agte"],            
...
if ($_POST["opcion"] === "MOS_INGXSUCURSAL") {
    ...
    while ($col = $query->fetch_array()) {
        $datos[] = array(
            ...
            "ing_efec_cta_agte" => $col["ing_efec_cta_agte"],
...
if ($_POST["opcion"] === "FLUJOCAJA") {
    $query = $mysqli->query("call spFlujoEfectivoCaja('" . $_POST["token"] . "')");
    $datos = array();
    while ($col = $query->fetch_array()) {
        $datos[] = array(
            "fecha" => $col["fecha"],
...
if ($_POST["opcion"] === "LIQUIDEZ") {
    $query = $mysqli->query("call spLiquidezAnterior('" . $_POST["fecha"] . "','" . $_POST["monto"] . "','" . $_POST["op"] . "')");
    $datos = array();
    while ($col = $query->fetch_array()) {
        $datos[] = array(
            "liquidez" => $col["liquidez"],
        );
    }
    $json = json_encode($datos);
    echo ( $json );
