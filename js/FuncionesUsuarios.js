...
console.log($('#txt_idbanco').val());
    if($('#txt_idbanco').val().length < 1){
        $('#txt_idbanco').val("0");
        console.log($('#txt_idbanco').val());
    }
    ...
    switch ($("#opproceso").val()) {
        case "I":
            ...
            {  /*opcion : opcion para el procedimeintos almacenado define a que tabla afecta( Usuario o Cliente), opproce: para listar o Insertar dentro del sp... */
                $.ajax({async: true, type: "POST", dataType: "json", cache: false,
                    data: {opcion: "U",nrocuenta: $('#txt_nrocuenta').val(), idbanco: $('#txt_idbanco').val(),idcliente:'0',idusuario: $('#idusuario').val(),
                    usuamodifica: $('#nick').val(),  opproce: $("#opproceso").val(),codempresa:$("#id_empresa").val()},
                    url: "controles/ManteBancos.php",
                    beforeSend: function (objeto) {$('.mensaje1').css('display', 'block'); $(".mensaje1").html("<img src='img/cargando.gif'>");},
                    complete: function (objeto) {$('.mensaje1').css('display', 'none');},
        ...
        case "L":
            $.ajax({async: true, type: "POST", dataType: "json", cache: false,
                data: {opcion:'U',nrocuenta:'0', idbanco:'0',idcliente:'0', idusuario: $('#idusuario').val(), usuamodifica: $('#nick').val(),opproce: $("#opproceso").val(),codempresa:$("#id_empresa").val()},
                url: "controles/ManteBancos.php",
                beforeSend: function (objeto) {$('.mensaje1').css('display', 'block');$(".mensaje1").html("<img src='img/cargando.gif'>");},
                complete: function (objeto) {$('.mensaje1').css('display', 'none');},
                success: fnCreaTablaUsuarioCuentas
...
$(document).ready(function(){
 $(function(){
  ...
  $('#txt_nrocuenta').validCampoFranz('0123456789-abcdefghijklmnopqrstuvwxyz');
