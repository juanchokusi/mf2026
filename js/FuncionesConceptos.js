        ...
        $.each(data, function (index, record) {
          if ($.isNumeric(index)) {
            ...
            var row = $(
              "<tr id='c[" +
                i +
                "]' onclick='fnSeleccionaConceptos(this.id);' > </tr>"
            );
            $(
              "<td>" +
                " <button id='btn_anular' onclick='fnAnulaConceptos();'  title='Anular' type='button' aria-hidden='true' class='btn btn-default btn-xs' ><span class='fa fa-times blue'></span></button>" +
                "</td>"
            )
              .text(record.itm2)
              .appendTo(row);
            $(
              "<td>" +
                " <button id='btn_editar' onclick='fnEditaConceptos();'  title='Editar' type='button' aria-hidden='true' class='btn btn-default btn-xs' ><span class='fa fa-pencil-square-o blue'></span></button>" +
                "</td>"
            )
              .text(record.itm3)
              .appendTo(row);
            $("<td> </td>").text(i).appendTo(row);
            $("<td class='ocultame'> </td>")
              .text(record.idconcepto)
              .appendTo(row);
        ...
        fnDrawCallback: function (oSettings) {
          ...
          if (oSettings.bSorted || oSettings.bFiltered) {
            for (var i = 0, iLen = oSettings.aiDisplay.length; i < iLen; i++) {
              $("td:eq(2)", oSettings.aoData[oSettings.aiDisplay[i]].nTr).html(
                i + 1
              );
            }
          }
...
function fnAnulaConceptos() {
  if ($("#tipo_usuario").val().trim() === "ADMIN") {
    jConfirm(
      "Seguro de Eliminar: " + $("#descripcion").val(),
...
$(document).ready(function () {
    ...
    if ($("#tipo_usuario").val().trim() === "ADMIN") {
      if (
        ...
        if (edita === "S") {
          ...
          jConfirm(
            "¿Esta seguro de agregar CONCEPTO: " + $("#descripcion").val(),
            "Giros - Transferencias",
            function (r) {
              if (r) {
                fnInsertaConcepto();
  ...
  $("#buscador").keyup(function () {
    if ($(this).val() !== "") {
      ...
      $("#mitable td:contains-ci('" + $(this).val() + "')")
        .parent("tr")
        .show();
    } else {
