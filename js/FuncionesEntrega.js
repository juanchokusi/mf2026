...
function eRecuperaFila(eidfila) {
    ...
    $("#nombre_archivo").val(elTableCells[26].innerHTML);
    $("#sele_fe").val(eidfila);
    ...
    console.log('codgirosucu_img:',$("#codgirosucu_img").val());
    console.log('correlativo_img:',$("#correlativo_img").val());
...
function CreaTablaEntregados(jsonrecibe) {
    ...
    for (var contador = 0; contador < jsonrecibe.length; contador++) {
        ...
        if (voucher === 'XXX') {
            ...
            html += "<td>" + " <button id=" + jsonrecibe[contador].voucher + " onclick='cambiaImagene(this.id);' title='Boucher' type='button' aria-hidden='true' class='btn btn-default btn-xs' ><span class='glyphicon glyphicon-picture blue'></span></button>" + "</td>";
        }
        ...
        html += "<td class='eoculto'>" + jsonrecibe[contador].voucher + "</td>";
        html += "</tr>";
    }
    ...
    if (remitente.length === 0 || beneficiario.length === 0 || destino.length === 0 || importe === 0)
    {
        ...
        $.ajax({async: true, type: "POST", cache: false, dataType: 'json',
            data: {opt: "I", fechagiro: fechagiro, origen: origen, remitente: remitente, destino: destino, beneficiario: beneficiario,
                importe: importe, cargo: cargo, igv: igv, itf: itf, otro: otro, total: total.toFixed(2) , efectivo: efectivo, ciudestino: ciudestino,
    ...
    switch (accion) {
        case "manualmente":
            if (confirma_impresion === "SI") {
                window.open('ImprimeTicketEntrega.php?pdfcodgirosucursal=' + $('#codgirosucursal').val() + '&pdfnick=' + $('#nick').val() + '&codsucu=' + $('#codsucursal').val(), '_blank');  // changed here (cambiado aquí)
                window.open('ImprimeTicket_Entrega.php?pdfcodgirosucursal=' + $('#codgirosucursal').val() + '&pdfnick=' + $('#nick').val() + '&codsucu=' + $('#codsucursal').val(), '_blank');  // changed here (cambiado aquí)
            } else {
                $.alert("NO tienes permiso para volver a imprimir, comunicate con el Administrador...", "Giros - Transferencias");
            }
            break;
        case "alguardar":
            window.open('ImprimeTicketEntrega.php?pdfcodgirosucursal=' + $('#codgirosucursal').val() + '&pdfnick=' + $('#nick').val() + '&codsucu=' + $('#codsucursal').val(), '_blank');
            window.open('ImprimeTicket_Entrega.php?pdfcodgirosucursal=' + $('#codgirosucursal').val() + '&pdfnick=' + $('#nick').val() + '&codsucu=' + $('#codsucursal').val(), '_blank');
    }
    return false;
}
...
//////////////// DIALOGOS ////////////////
    ...
    $("#dialogo_asigcuenta").dialog({autoOpen: false, resizable: true,
        ...
        buttons: {
            ...
            'Pagar': function () {
                if ($("#nrodeoperacion").val().trim() !== '' && $("#nrodeoperacion").val().length > 5 && $('#datoscuenta').val().trim() !== '') {
                    fnPagar();
                    $(this).dialog("close");
                } else {
                    $.alert({ title: "Money Flash", content: "Datos invalidos, Verifique...", type: "red" });
                }
            },
            'Cancelar': function () {
                $(this).dialog("close");
    ...
    $("#dialogo_pagaagente").dialog({autoOpen: false, resizable: true,
        ...
        buttons: {
            'Aceptar': function () {
                if ($("#nro_deoperacion").val().trim() !== '' && $("#nro_deoperacion").val().length > 5 && $('#datos_cuenta').val().trim() !== '') {
                    fnPagarConAgente();
                    $(this).dialog("close");

                } else {
                    /* jAlert("Completa la informacion requerida, Verifique...", "Giros - Transferencias"); */
                    $.alert({ title: "Money Flash", content: "Completa la informacion requerida, Verifique...", type: "red" });
                }
            },
            'Cancelar': function () {
                $(this).dialog("close");
    ...
    $("#dialogo_paga_con_sucu").dialog({autoOpen: false, resizable: true,
        ...
        open: function (event, ui) {
            ...
            $("#nrodeoperacion").val("");
            $("#txt__sucursal").val("");
            $("#txt__sucursal").attr("disabled", true);
            fnMuestraSucursales();
        },
        buttons: {
            'Aceptar': function () {
...
//      $("#tabla_bancos").css("display", "block");
    ...
    $("#diag_SaldoDisponible").dialog({autoOpen: false, resizable: true,
        modal: true, height: 500, width: 550, show: {effect: "blind", duration: 500}, hide: {effect: "fade", duration: 500},
        open: function (event, ui) {
    ...
    $("#boton_pagar").click(function () {
        if ($("#usuaentrega").val() === '---') {
            if ($("#enombresb").val().trim() === '' && $("#enombresr").val().trim() === '')
            {
                ...
                $.alert({title: "Money Flash", content: "Seleccione un cliente...", type: "red" });
            } else {
                $("#grupo").val("C");
            ...
            $.alert({title: "Money Flash", content: "Ya esta pagado, Seleccione otro ..." + $("#usuaentrega").val(), type: "red" });
        }
    });
...
$("#btn_imprime_voucher").click(function () {
    ...
    $('#btn_elimina_voucher').click(function () {
...
$('#uploadBtn').on('click', function() {
    ...
    var valor1 =  $("#codgirosucu_img").val().trim();
    newFileName = valor1;
    ...
    $.ajax({
        url: 'php/upload_voucher.php',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        success: function(response) {
            /* $.alert({ title: 'Resultado jojo', content: response.message, type: response.success ? 'green' : 'red'  }); */
            fnInsertaVoucher(newFileName);
            $('#fileInput').val('');
            $('#newFileName').val('');
            $('#uploadPreview').empty();
            $('#datos_cuenta').val('');
...
$("#imageModal").dialog({
  ...
  $("#carga_imagen").click(function () {
    const fileName = $("#nombre_archivo").val().trim();
    const deliveryStatus = $("#md_fechaentrega").val();
    // Validate required data
    if (!fileName) {
        $.alert({ title: "Money Flash", content: "Seleccione una Transferencia...", type: "red" });
        return;
    }
    // Check delivery status
    if (deliveryStatus === "Pendiente") {
        $.alert({title: "Money Flash", content: "Transferencia está pendiente, Verifique...", type: "orange" });
        return;
    }
    // Log for debugging
    ...
    try {
        ...
        $.alert({ title: "Error",  content: "No se pudo abrir el visor de imágenes", type: "red" });
    }

  });

$('#nrodeoperacion').on('input', function () { 
    // Reemplaza cualquier carácter que no sea un número por nada
    this.value = this.value.replace(/[^0-9]/g, '');
});
$('#nro_deoperacion').on('input', function () { 
    this.value = this.value.replace(/[^0-9]/g, '');
});
/* ============== fin muestra voucher =============  */
...
function fnMuestraCuentasxUsuario() {
    var idempresa1 = $('#codsucursal').val();
    idempresa1 = idempresa1.substring(0,1);
    $.ajax({async: true, type: "POST", dataType: "json", cache: false,
        data: {opcion: "CxU2", valor: $("#idbanco").val(), idempresa: idempresa1, opsql: 'L'},
        url: "controles/ManteBancos.php",
...
function fnSeleccionaSucursales(idfila) {
    ...
    $("#txt__sucursal").val(elTableCells[1].innerHTML+'-'+elTableCells[2].innerHTML);
            ...
            if (r) {
                $.ajax({type: "POST", url: "controles/ManteRecibidos.php",
                    data: {opt: 'PAGA', idgiro: $('#correlativo').val(), nrooperacion: $('#nro_deoperacion').val(), usuaentrega: $('#nick').val(),
                        sucuentrega: $('#codsucursal').val(), datospago: $('#datos_cuenta').val(), idempresa: $('#idempresa').val(), monto: $('#eimporte_r').val(), opsql: 'decuenta'},
            ...
            if (r) {
                $.ajax({type: "POST",
                    data: {opt: 'PAGA', idgiro: $('#correlativo').val(), nrooperacion: $('#nrodeoperacion').val(), usuaentrega: $('#nick').val(),
                        sucuentrega: $('#codsucursal').val(), datospago: $('#datoscuenta').val(), monto: $('#eimporte_r').val(),idempresa:$('#idempresa').val(), opsql: 'decuenta'},
                    url: "controles/ManteRecibidos.php",
                    success: fnCreaNuevoDiario
                }).done(function (msg) {
...
function fnMuestraFlujoSucursales() {
    $.ajax({async: true, type: "POST", dataType: "json", cache: false,
        data: {opt: 'FLUJOSUCURSALES', mitoken: $('#mi_token').val()},
        url: "controles/ManteRecibidos.php",
        beforeSend: function (objeto) {
...
function fnIngresosxSucursal() {
    console.log($('#mi_token').val());
    $.ajax({async: true, type: "POST", dataType: "json", cache: false,
        data: {opt: 'INGRESOSXSUCURSAL', fechai: $('#efecha_r').val(), fechaf: $('#efecha_r').val(), mitoken: $('#mi_token').val(),idempresa: $('#codsucursal').val()},
        url: "controles/ManteRecibidos.php",
        beforeSend: function (objeto) {$('#carga_fs').css('display', 'block');},        
        success: function(mirpta) {
        console.log("ARTICULO: " + mirpta.rpta);
       fnEgresosxSucursal();
      }
    });
    return false;
...
function fnEgresosxSucursal() {
    console.log($('#mi_token').val());
    $.ajax({async: true, type: "POST", dataType: "json", cache: false,
        data: {opt: 'EGRESOSXSUCURSAL', fechai: $('#efecha_r').val(), fechaf: $('#efecha_r').val(), mitoken: $('#mi_token').val(),idempresa: $('#codsucursal').val()},
        url: "controles/ManteRecibidos.php",
        beforeSend: function (objeto) {        },
        success: fnMuestraFlujoSucursales
    });
    return false;
}
...
function CreaTablaFlujoSucursales(json) {
    ...
    for (var x = 0; x < json.length; x++) {
        ...
        html += "<td align='right'>" + json[x].ingresos + "</td>";
        html += "<td align='right'>" + json[x].egresos + "</td>";
...
function fnInsertaVoucher(newfilename) {
    $.ajax({async: true, type: "POST", dataType: "json", cache: false,
        data: {opt: 'VOUCHER', correlativo: $("#correlativo_img").val(), codgirosucu: $('#codgirosucu_img').val(), fechai: $('#efechahoy').val(), fechaf: $('#efechahoy').val(),
            usuariocarga: $('#nick').val(), descripcion: newfilename, usuarioimprime: $('#nick').val(), op: 'I'},
        url: "controles/ManteRecibidos.php",
        success: function () {   
            $.alert({ title: "Archivo Cargado, Verifique...", content: "Money - Flash.", type: "green" });
            $("#correlativo_img").val("");
            $("#codgirosucu_img").val("");
            $("#descripcion_img").val("");
...
function fnEliminaVoucher(nombre_archivo) {
  $.ajax({
    async: true, type: "POST", dataType: "json", cache: false,
    data: { opt: 'ELIMINA_VOUCHER', nombrearchivo: nombre_archivo },
    url: "controles/ManteRecibidos.php",
    success: function () {
      $("#TEntregados").dataTable().fnDestroy();
      MuestraEntregados($('#efecha_r').val(), 'E');
      $.alert({ title: "Archivo Eliminado, Verifique... fnEliminaVoucher", content: "Money - Flash.", type: "green" });
      $("#correlativo_img").val("");
      $("#codgirosucu_img").val("");
      $("#descripcion_img").val("");
      $("#dialogo_voucher_imge").dialog("close");
...
/* async function fnEliminaVoucher(nombre_archivo) {
  try {
      // Make the AJAX call using Promise
      ...
      jAlert("Archivo Eliminado, Verifique...", "Money Flash");
      ...
      // Refresh data table
      $("#TEntregados").dataTable().fnDestroy();
      await MuestraEntregados($('#efecha_r').val(), 'E');

  } catch (error) {
      // Handle any errors
      console.error("Error eliminando voucher:", error);
      jAlert("Error al eliminar el archivo", "Money Flash");
  }
} */
            ...
            if (confirm("¿Estás seguro de que quieres eliminar esta imagen?")) {
              $.ajax({
                ...
                success: function (deleteResponse) {
                  ...
                  fnEliminaVoucher(nombre_archivo);
                  /* $.alert({ title: deleteResponse, content: "Money - Flash.", type: "red" }); */
                },
                error: function () {
              ...
              $.alert('No tienes permiso, comunicate con el Administrador...', 'Money-Flash', 'red  ');
            }
          });
