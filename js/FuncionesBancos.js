function fnInsertaBanco() {
  var cabecera =
    " <tr> <th>Itm</th> <th>Descripcion</th> <th>Iniciales</th> <th>Direccion</th> <th>WEB</th> <th>Telefono</th> <th>Grupo</th> <th> </th> </tr> ";
  ...
  $.ajax({
    ...
    data: {
      opcion: "INSERTA",
      descripcion: $("#desc_banco").val(),
      iniciales: $("#inicial_banco").val(),
      direccion: $("#direccion_banco").val(),
      web: $("#web_banco").val(),
      telefono: $("#fono_banco").val(),
      grupo: $("#select_tipocuenta").val(),
      nrocuenta: nrocuenta_agente,
      idempresa: $("#id_empresa").val(),
      usuario: $("#idusuario").val(),
      usuario_modifica: $("#idusuario").val(),
    },
    ...
    for (var i = 0; i < json.length; i++) {
      ...
      $(".editinplace").append(
        "<tr id='b[" +
          i +
          "]' onclick='fnSeleccionaBancos(this.id);'>" +
          "<td >" +
          x +
          "</td>" +
          "<td class ='ocultame id'>" +
          json[i].idbanco +
          "</td>" +
          "<td class='editable' data-campo='desc_banco'><span>" +
          json[i].desc_banco +
          "</span></td>" +
          "<td class='editable' data-campo='iniciales'><span>" +
          json[i].iniciales +
          "</span></td>" +
          "<td class='editable' data-campo='direccion'><span>" +
          json[i].direccion +
          "</span></td>" +
          "<td class='editable' data-campo='web'><span>" +
          json[i].web +
          "</span></td>" +
          "<td class='editable' data-campo='telefono'><span>" +
          json[i].telefono +
          "</span></td>" +
          "<td class='editable' data-campo='grupo'><span>" +
          json[i].grupo +
          "</span></td>" +
          "<td>" +
          "<button id='btn_anular' onclick='fnAnulaBanco();' title='Anular' type='button' aria-hidden='true' class='btn btn-default btn-xs' ><span class='glyphicon glyphicon-trash blue'></span></button>" +
          "</td>" +
          "</tr>"
      );
    }
...
function InsertaBanco() {
  $.ajax({
    ...
    data: {
      ...
      telefono: $("#fono_banco").val(),
      grupo: $("#select_tipocuenta").val(),
      nrocuenta: $("#nrocuenta_agente").val(),
      idempresa: $("#id_empresa").val(),
      usuario: $("#idusuario").val(),
      usuario_modifica: $("#idusuario").val(),
    },
...
function fnAnulaBanco() {
  if ($("#tipo_usuario").val().trim() === "ADMIN") {
    jConfirm(
      "Seguro de Eliminar: " + $("#descbanco").val(),
...
//Para escribir solo letras
 $(function(){
  //Para escribir solo letras
  $('#nrocuenta_agente').validCampoFranz('0123456789-abcdefghijklmnopqrstuvwxyz');
  $('#desc_banco').validCampoFranz('0123456789-abcdefghijklmnopqrstuvwxyz');
  $('#inicial_banco').validCampoFranz('0123456789-abcdefghijklmnopqrstuvwxyz');
});
    ...
    for (var i = 0; i < json.length; i++) {
      ...
      $(".editinplace").append(
        "<tr id='b[" +
          ...
          "</span></td>" +
          "<td>" +
          "<button id='btn_anular' onclick='fnAnulaBanco();' title='Anular' type='button' class='btn btn-default btn-xs' ><span class='glyphicon glyphicon-trash blue'></span></button>" +
          "</td>" +
          "</tr>"
      );
    }
  ...
  $(document).on("dblclick", "td.editable span", function (e) {
    if ($("#tipo_usuario").val().trim() === "ADMIN") {
      ...
      if (campo !== "grupo") {
        ...
        td.text("").html(
          "<select name='grupo' ><option value='C'>CUENTA</option><option value='A'>AGENTE</option></select><a class='enlace guardagrupo' href='#'>GuardaGrupo</a> <a class='enlace cancelar' href='#'>Cancelar</a>"
        );
      }
    } else {
      jError("Solo Administrador", "Giros - Transferencias");
  ...
  $("#btn_guardar").click(function () {
    if ($("#select_tipocuenta").val() === "A") {
      if (
        $("#desc_banco").val().trim() === "" ||
        $("#inicial_banco").val().trim() === "" ||
        $("#nrocuenta_agente").val().length < 9
      ) {
  ...
  $("#buscador").keyup(function () {
    if ($(this).val() !== "") {
      ...
      $("#tabla_bancos td:contains-ci('" + $(this).val() + "')")
        .parent("tr")
        .show();
    } else {
