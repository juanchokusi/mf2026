...
$sql_empresa = "SELECT idempresa,descripcion from empresa order BY descripcion";
$result_empresa = $mysqli->query($sql_empresa);
$optempresa = '<option value="0">Elige Empresa...</option>';
while ($fila = $result_empresa->fetch_array(MYSQLI_ASSOC)) {
    $optempresa.='<option value="'.$fila["idempresa"].'">' . $fila["descripcion"] . '</option>';
}
...
<input type="hidden" id="id_empresa" value="<?php echo $_SESSION['idempresa'] ?>">
